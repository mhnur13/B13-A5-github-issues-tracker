...
const displayModal = (item) => {
  ...
  if (item.priority == "high") {
    priorityDesign =
      "py-1 px-6 bg-red-500 text-white border-none rounded-full text-[14px]";
  } else if (item.priority == "medium") {
    priorityDesign =
      "py-1 px-6 bg-amber-500 text-white border-none rounded-full text-[14px]";
  } else {
    priorityDesign =
      "py-1 px-6 bg-gray-400 text-white border-none rounded-full text-[14px]";
  }
...
function displayAllCards(data) {
  ...
  document.getElementById("issueNumber").innerText =
    `${mainSection.children.length} Issues`;
  hideSpinner();
}
